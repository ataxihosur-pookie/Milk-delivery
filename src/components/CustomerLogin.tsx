...
import { Milk, Eye, EyeOff, ShoppingCart } from 'lucide-react';
...
const CustomerLogin: React.FC<CustomerLoginProps> = ({ onLogin }) => {
  ...
  const { authenticateCustomer } = useData();
  ...
  if (showSignup) {
    return <CustomerSignup onBack={() => setShowSignup(false)} onSignupSuccess={() => setShowSignup(false)} />;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="mt-6 text-3xl font-extrabold text-gray-900">MilkChain Customer</h2>
          <p className="mt-2 text-sm text-gray-600">Customer Portal Login</p>
        </div>
        ...
        <form className="mt-8 space-y-6 bg-white p-8 rounded-lg shadow-lg" onSubmit={handleSubmit}>
          ...
          <div className="mt-6 text-xs text-gray-500 space-y-2">
            ...
            <div>Use your registered phone number and password</div>
          </div>
        </form>
      </div>
    </div>
